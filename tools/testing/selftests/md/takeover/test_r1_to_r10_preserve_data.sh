...
# Empirical check of the byte-identity invariant the takeover relies
# on: for near_copies == raid_disks the raid10_near layout resolves
# every logical sector to the same physical offset raid1 used, so the
# array contents must be bit-for-bit identical across the swap.
. "$(dirname "$0")/lib.sh"
md_require_root
md_require_tools
md_require_modules
