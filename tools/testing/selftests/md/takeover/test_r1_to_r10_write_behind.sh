...
# Write-behind precondition: max_write_behind != 0 must be rejected.
. "$(dirname "$0")/lib.sh"
md_require_root
md_require_tools
md_require_modules
...
# A write-behind member is also write-mostly, so the write-mostly check
# would fire first. To isolate the write-behind check, clear WriteMostly
# on both members first.
