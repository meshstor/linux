...
[ "$REMAINING" -le 0 ] || { gds_verdict p7 p7d FAIL "remaining=$REMAINING after a failed write"; echo "FAIL: strict remaining=1 budget not all spent (<=0: all spent or over — injector counter is racy under 4-worker completion)" >&2; exit 1; }
