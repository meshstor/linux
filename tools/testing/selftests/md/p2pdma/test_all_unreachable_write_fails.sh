...
# NO leg is P2P-reachable from the CMB: the write must fail -EINVAL and
# fault nothing. On the UNFIXED base this observes the bug in its worst
# form: the write reports SUCCESS while NO leg got the data (total loss).
set -u
...
[ "$RC" = "-22" ] || p2p_fail "all-unreachable p2p write returned errno=$RC, want -22 (EINVAL)"
...
p2p_pass "all-unreachable p2p write failed loud, faulted nothing"
