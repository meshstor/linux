...
# raid10 arm of the core bug: 2-device raid10 (mirrored layout), leg2
# P2P-unreachable from the CMB. The write must fail -EINVAL, fault
# nothing, and record no badblocks. On the UNFIXED base this observes
# the bug: the write reports SUCCESS (silent divergence).
set -u
...
p2p_load_ms_modules
ARR="$(p2p_make_array 10 "$(p2p_nvme_by_serial leg1)" "$(p2p_nvme_by_serial leg2)")"
RC="$(p2p_io write "$ARR" "$(p2p_bdf_by_serial cmb0)")"
[ "$RC" = "-22" ] || p2p_fail "raid10 asymmetric p2p write returned errno=$RC, want -22 (EINVAL)"
...
p2p_pass "raid10 asymmetric p2p write failed loud, faulted nothing"
