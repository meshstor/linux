...
# on a kernel without the SED code the feature-detect check skips them.
# MD_SUBSYS retargets the suite -- MD_SUBSYS=md selects the in-tree md
# driver -- and individual knobs (MDADM, LATBAL_*) override the
...
#   LATBAL_SUBSYS=foo              # pass --subsys=foo to mdadm
#                                  # (default $MD_SUBSYS; none under md)
#   LATBAL_DEV_PREFIX=foo          # device basename prefix (default $MD_SUBSYS)
#   LATBAL_SYSFS_SUBDIR=foo        # /sys/block/<dev>/<subdir> (default $MD_SUBSYS)
#   LATBAL_MDSTAT=/proc/foostat    # personality file (default /proc/${MD_SUBSYS}stat)
#
...
# driver.  Each LATBAL_* still overrides when set explicitly.
MD_SUBSYS="${MD_SUBSYS:-ms}"
if [ "$MD_SUBSYS" = md ]; then
	LATBAL_SUBSYS="${LATBAL_SUBSYS:-}"
else
	LATBAL_SUBSYS="${LATBAL_SUBSYS:-$MD_SUBSYS}"
fi
LATBAL_DEV_PREFIX="${LATBAL_DEV_PREFIX:-$MD_SUBSYS}"
LATBAL_SYSFS_SUBDIR="${LATBAL_SYSFS_SUBDIR:-$MD_SUBSYS}"
LATBAL_MDSTAT="${LATBAL_MDSTAT:-/proc/${MD_SUBSYS}stat}"
# Prefer a locally installed mdadm (for an out-of-tree stack, the one
...
# the globals).  A stale-but-clear node is fair game for reuse.
latbal_alloc_md() {
