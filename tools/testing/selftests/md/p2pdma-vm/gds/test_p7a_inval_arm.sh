...
# P7a: THE ARM — stage-1 fail-the-write on real GPU I/O, raid1, INVAL.
# CSI raid1 over two local NVMe partitions, strict cufile.json, witnessed
# gdsio write, injector match=success to_status=inval p2p_only=1 remaining=1
# on one leg. Assert: gdsio FAILS (EINVAL surfaced through cuFile); [UU]
# preserved (no member faulted); bad_blocks CONTENT empty on both legs;
# "no P2P path" breadcrumb in the dmesg delta; witness saw P2P bios;
# injected>=1 && remaining==0; array stops cleanly (timeout-guarded).
set -eu
...
JSON=$(gds_cufile_json strict "$GDS_RESULTS/p7a")
...
rc=0; "$WITNESS" --expect-ms nonzero --expect-rc nonzero -o "$GDS_RESULTS/p7a-witness.txt" -- \
...
case $rc in
	...
	*) gds_verdict p7 p7a FAIL "want p2p_bios>0 AND gdsio rc!=0; got: $(head -1 "$GDS_RESULTS/p7a-witness.txt" 2>/dev/null)"
	   echo "FAIL: stage-1 arm did not surface EINVAL to a witnessed native write" >&2; exit 1;;
esac
...
gds_assert_no_badblocks /dev/ms0 \
	|| { gds_verdict p7 p7a FAIL "badblocks recorded"; echo "FAIL: arm must record no badblocks" >&2; exit 1; }
gds_assert_breadcrumb \
	|| { gds_verdict p7 p7a FAIL "breadcrumb absent"; echo "FAIL: 'no P2P path' breadcrumb missing" >&2; exit 1; }
gds_injector_unload
umount "$GDS_MNT"
timeout 60 "$MDADM" --stop /dev/ms0 >/dev/null 2>&1 \
	|| { gds_verdict p7 p7a FAIL "mdadm --stop timed out (writes_pending imbalance?)"; echo "FAIL: array did not stop cleanly" >&2; exit 1; }
P2PDMA_ARRAY=""
gds_verdict p7 p7a PASS "EINVAL surfaced injected=$INJECTED [UU] badblocks-empty breadcrumb"
echo "PASS: stage-1 INVAL arm on raid1 (injected=$INJECTED, write failed loud, no fault, no badblocks, breadcrumb)"
exit 0
