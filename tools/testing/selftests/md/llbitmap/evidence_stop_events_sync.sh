...
#
# Evidence probe: does legacy_async_del_gendisk=Y cause member
...
# investigating events-counter divergence. Exit code is always 0 (a probe).
# The caller decides from the JSON summary whether the deltas justify
# flipping the legacy_async_del_gendisk default.
#
# Decision rule:
#   the default flip is justified iff max(delta_N) == 0 AND
#   max(delta_Y) >= 2 across >= 20 trials.
...
# by its evidence_*.sh name (see the header). The decision rule
