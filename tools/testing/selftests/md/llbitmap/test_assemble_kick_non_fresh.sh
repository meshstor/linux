...
#
# Kick-non-fresh reproducer (stale member on re-assemble):
...
# The kick itself is longstanding behavior; the assertion on the
# enriched warning format covers "md: log events counter values when
# kicking non-fresh device". Flipping the legacy_async_del_gendisk
# default must not affect this test either - that flag only changes
# the events-divergence race, not the kick path.
...
# With "md: log events counter values when kicking non-fresh device"
# the warning carries the actual events numbers. Require that format
# here; a kernel without that change fails this assertion.
