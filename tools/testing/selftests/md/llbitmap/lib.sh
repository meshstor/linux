...
#
# BOTH subsystems need a patched mdadm: the llbitmap arrays are
...
# Path to a patched mdadm (understands --bitmap=lockless and the
...
llbitmap_require_modules() {
	if [ ! -x "$MDADM" ]; then
		echo "SKIP: patched mdadm not found at $MDADM" >&2
		exit 4
	fi
