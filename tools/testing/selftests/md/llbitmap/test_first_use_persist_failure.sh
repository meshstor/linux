...
#      persist FIRST_USE clear, every rdev write fails.  The fix's
#      md_write_super_acct() detects total failure, BITMAP_WRITE_ERROR
#      is set, and llbitmap_init() returns -EIO; RUN_ARRAY fails.
#
# Verdict:
#   PASS  mdadm reports "failed to RUN_ARRAY ...: Input/output error"
#         OR dmesg shows "failed to persist initial bitmap super".
...
# Inside llbitmap_init it calls llbitmap_update_sb → llbitmap_write_sb,
# which writes to both rdevs (both fail).
...
# OR dmesg shows the "failed to persist initial bitmap super" message.
...
FIX_SIGNAL=0
if echo "$out" | grep -qiE 'failed to RUN_ARRAY|Invalid argument|started.*0 drives'; then
	FIX_SIGNAL=1
fi
if sudo dmesg | tail -50 | grep -q 'failed to persist initial bitmap super'; then
	FIX_SIGNAL=1
fi
...
if [ "$A_STILL" -ne 0 ] || [ "$B_STILL" -ne 0 ]; then
	...
	echo "  Need md_write_super_acct()-based total-write-failure detection"
