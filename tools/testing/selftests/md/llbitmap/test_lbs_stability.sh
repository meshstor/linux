...
#
# The other reproducers in this suite are create-only and pin every
# member's logical block size, so they never exercise `mdadm --assemble`
# and never let member geometry vary across the stop/assemble boundary.
#
# Kernels whose mdp_superblock_1 lacks the logical_block_size field do
# NOT persist an array's logical block size: it is recomputed as max()
# over the member devices on every md_run(). The 6.18 superblock field
# exists to make it stable instead.
#
...
#       This is the assertion that fails on a build that wrongly compiles
#       the field support out on a kernel whose superblock carries it.
#       SKIPPED on pre-6.18 kernels.
#
#   field ABSENT   (pre-6.18 kernels):
...
# Struct-scoped scan of the running kernel's mdp_superblock_1 for a
# logical_block_size member (not a version check: backports count).
sb_lbs_field_state() {
	local hdr
