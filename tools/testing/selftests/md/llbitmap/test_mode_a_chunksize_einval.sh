...
#
# Corrupted-bitmap-super reproducer: a chunksize that is a power of 2
# but too small to fit the array's chunks in the reserved space must be
# rejected at assemble with a clear diagnostic.
#
# This test forces the dimensional chunksize check in md-llbitmap.c's
# llbitmap_read_sb() by writing chunksize=1 sector directly into the
# on-disk bitmap super. For a 100 MiB v1.2 array with mdadm's typical
# sectors_reserved=256 (131072 bytes reserved):
#   chunks_needed = DIV_ROUND_UP(204800, 1) = 204800
#   bytes_reserved = 131072
#   204800 > 131072 → must be rejected
#
# The kernel's terse diagnostic compares the chunksize against a
# derived lower bound ("chunksize too small 1 < ..."); an enriched
# diagnostic would name the sizes directly ("needs 204800 bytes,
# 131072 reserved"). The test requires the rejection either way and
# accepts both dmesg formats, reporting which one fired.
...
# the is_power_of_2 check passes. But for a 100 MiB v1.2
...
#   204800 > 131072 → must be rejected
# This exercises the dimensional check.
