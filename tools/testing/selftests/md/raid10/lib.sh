...
# MD_SUBSYS retargets the suite -- MD_SUBSYS=md selects the in-tree
# md/raid10 driver -- and the individual RAID10_* / MDADM knobs
# override the per-subsystem defaults when set explicitly, e.g.:
#
#   MD_SUBSYS=ms                   # driver stack under test (the default)
#   MDADM=/usr/local/bin/mdadm     # mdadm binary (the default; PATH fallback)
#   RAID10_DEV_PREFIX=foo          # device basename prefix (default $MD_SUBSYS)
#   RAID10_SYSFS_SUBDIR=foo        # /sys/block/<dev>/<subdir> (default $MD_SUBSYS)
#   RAID10_MDSTAT=/proc/foostat    # personality file (default /proc/${MD_SUBSYS}stat)
#
...
# device/sysfs/stat knobs.  'ms' (the default) and any other value <s>
# target an md-compatible stack by convention -- /dev/<s>N device
# nodes, /sys/block/<s>N/<s> sysfs and /proc/<s>stat -- while 'md'
# selects the in-tree driver.  Each RAID10_* still overrides when set
# explicitly.
MD_SUBSYS="${MD_SUBSYS:-ms}"
RAID10_DEV_PREFIX="${RAID10_DEV_PREFIX:-$MD_SUBSYS}"
RAID10_SYSFS_SUBDIR="${RAID10_SYSFS_SUBDIR:-$MD_SUBSYS}"
RAID10_MDSTAT="${RAID10_MDSTAT:-/proc/${MD_SUBSYS}stat}"
# Prefer a locally installed mdadm (for an out-of-tree stack, one that
# understands its device-name prefix); fall back to the system binary.
if [ -z "${MDADM:-}" ]; then
...
# An out-of-tree stack may leave a stale but empty device node behind
# after --stop; we accept it if array_state is "clear".
raid10_alloc_md() {
	local n
	for n in $(seq 240 255); do
		...
		if [ -b "$dev" ]; then
			...
			# After --stop the node may persist (out-of-tree
			# stacks); only reuse it if there's no live array.
			local state
...
# Sysfs base for the device under test (e.g. /sys/block/md240/md).
_raid10_sysfs() {
