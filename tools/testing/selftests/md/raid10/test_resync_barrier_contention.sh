...
#
# Before the per-bucket conversion (commit 8959e7ea + fix f5068df5):
#   struct r10conf {
...
trial() {
	raid10_wait_idle "$MD"
	local idle_line
	idle_line="$(raid10_fio_p99_iops "$DEV" "$FIO_SECS" "$NJOBS")"
	raid10_wait_idle "$MD"
	...
	sleep 1
	local resync_line
	resync_line="$(raid10_fio_p99_iops "$DEV" "$FIO_SECS" "$NJOBS")"
	raid10_stop_sync "$MD"
	raid10_wait_idle "$MD"
...
mid = n // 2
ratio = resync[mid] / idle[mid] if idle[mid] else 0
print(f"{ratio:.3f}")
' "${IDLE_P99[@]}" "${RESYNC_P99[@]}")"
