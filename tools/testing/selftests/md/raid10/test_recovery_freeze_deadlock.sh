...
# vs freeze_array() deadlock.  On a kernel without the forced-raise back-off
# fix that wedge is real: ${MD}_resync parks in raise_barrier() and
# ${MD}_raid10 in freeze_array(), both in D state, and only a reboot clears
# them.  Refuse to run against the in-tree driver, whose running kernel may
# predate the fix, so a bare run of the suite can never brick the host; opt
# in by pointing MD_SUBSYS at an out-of-tree build known to carry it.
if [ "$MD_SUBSYS" = md ]; then
	raid10_skip "this reproducer wedges kernels without the forced-raise back-off (D-state kthreads, reboot needed); refusing the in-tree driver -- set MD_SUBSYS to a fixed out-of-tree stack to opt in"
fi
