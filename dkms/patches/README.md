...
- **`<name>.patch.when`** — a guard evaluated by `bin/build-tarball`,
  ...
  `0010` only when it is absent. `0011` (the stage-1 fail-the-write completion
  arms) applies only when `R10BIO_P2PError` is present, so it self-excludes from
  an advertise-only pin that advertises P2PDMA but carries no fail-the-write
  arms (the gds1 kit variant), where its hunks would reject against absent code.
...
| `0008-p2pdma-feature-flag-gating.patch` _(`.when`: `raid1_can_advertise_p2pdma` present)_ | gates the P2P **advertise** machinery (feature branch `p2pdma-fixes`: member-AND advertise + `raid1_can_advertise_p2pdma()`, `md_bio_is_p2pdma()`, the add-path re-gate `raid1_p2pdma_clear_on_add()`, `REQ_NOMERGE` preserve, write-behind skip) so it compiles out on pre-6.11 kernels | Source-level, `#ifdef HAVE_BLK_FEAT_PCI_P2PDMA`. On kernels without the flag (e.g. RHEL9/5.14) the helper and both advertise sites `#ifdef` out and `md_bio_is_p2pdma()` folds to false. The feature *logic* is a feature branch, not a patch; only this capability-gating is compat. Present in BOTH featured kit variants (gdsM full stage-1 + gds1 advertise-only pin); the stage-1 fail-the-write completion arms are gated separately in `0011`. Guarded on `raid1_can_advertise_p2pdma` present (composed meshstor-main / p2pdma pins; on bare upstream 0009 handles it) |
| `0011-p2pdma-failwrite-arm-gating.patch` _(`.when`: `R10BIO_P2PError` present)_ | gates the P2P **stage-1 fail-the-write completion arms** (feature branch `p2pdma`: the `R1BIO_P2PError`/`R10BIO_P2PError` `end_write_request` arms, the `call_bio_endio`/`raid_end_bio_io` master-status overrides, both `end_read_request` read-fidelity blocks) so they compile out on pre-6.11 kernels | Source-level, `#ifdef HAVE_BLK_FEAT_PCI_P2PDMA`. Split out of 0008 (spec §6 amendment 2026-07-03): the arm code exists ONLY in the full stage-1 tree, so this patch is guarded on `raid10.c:R10BIO_P2PError` and self-excludes from an advertise-only pin (advertises P2PDMA, fail-the-write absent — the gds1 variant), where these hunks would reject against absent code. Hand-maintained (`.when`); the raid10 read-fidelity hunk keeps minimal symmetric context because md-latency-ewma inserts a fold-kick block above the anchor in composed meshstor-main |
